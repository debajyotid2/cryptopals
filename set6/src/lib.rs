...
use set2::generate_random_bytevec;
use set4::{sha1, BitVec};
use set5::*;
...
fn bigint(bytes_be: &[u8]) -> BigInt {
    BigInt::from_bytes_be(Sign::Plus, bytes_be)
}

fn bytearray(integer: &BigInt) -> Vec<u8> {
    integer.to_bytes_be().1
}
...
fn bigint_div_ceil(numerator: &BigInt, denominator: &BigInt) -> BigInt {
...
fn bigint_div_floor(numerator: &BigInt, denominator: &BigInt) -> BigInt {
...
mod test {
    ...
    fn test_bigint_div_ceil() {
        assert_eq!(bigint_div_ceil(&BigInt::from(10000002), &BigInt::from(10000)), BigInt::from(1001));
    }

    #[test]
    fn test_bigint_div_floor() {
        assert_eq!(bigint_div_floor(&BigInt::from(10000002), &BigInt::from(10000)), BigInt::from(1000));
    }
}
