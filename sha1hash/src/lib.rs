...
pub fn sha1_hash(
    ...
    for chunk in message.get_data().chunks(512) {
        ...
        for count in 16..80 {
            let res =
                (words[count - 3] ^ words[count - 8]) ^ (words[count - 14] ^ words[count - 16]);
        ...
        for count in 0..80 {
            ...
            let tmp: u32 = ((u32_rotate_left(&a, 5) as u64
                + f as u64
                + e as u64
                + k as u64
                + words[count] as u64)
...
pub fn sha1(message_arg: &BitVec) -> String {
    ...
    // Preprocessing
    message.extend(
        compute_md_padding_sha1(message_arg.len())
            .get_data()
            .clone(),
    );
...
mod tests {
    ...
    fn test_sha1() {
        let message1 =
            BitVec::new_from_bytearray(&b"The quick brown fox jumps over the lazy dog".to_vec());
