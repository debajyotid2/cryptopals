use aescipher::{
    aes_cbc_decrypt, aes_ecb_encrypt, detect_aes_ecb_encryption, generate_random_bytevec,
};
...
pub fn pad_random_bytes(plaintext: &Vec<u8>) -> Vec<u8> {
    ...
    padded_plaintext.splice(
        0..0,
        (generate_random_bytevec(padding_size_prefix))[..]
            .iter()
            .cloned(),
    );
    padded_plaintext.splice(
        padded_plaintext.len()..padded_plaintext.len(),
        (generate_random_bytevec(padding_size_suffix))[..]
            .iter()
            .cloned(),
    );
...
pub fn parse_key_value(string: &str) -> Option<HashMap<String, String>> {
    ...
    let parts: Vec<Option<(&str, &str)>> = res.iter().map(|part| part.split_once("=")).collect();
