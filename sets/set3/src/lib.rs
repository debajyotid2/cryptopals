...
use aescipher::{aes_ctr_decrypt, generate_random_bytevec};
...
pub fn get_ctr_encryptor() -> impl Fn(&Vec<u8>) -> Vec<u8> {
    ...
    let dumb_ctr_encryptor =
        move |plaintext: &Vec<u8>| -> Vec<u8> { aes_ctr_decrypt(plaintext, &key, &nonce) };
