...
fn challenge_25() {
    let plaintext: Vec<u8> = base64tobytearray(
        &fs::read_to_string("25.txt")
    ...
    let editing_api_call =
        |ciphertext_arg: &Vec<u8>, offset: usize, newtext: &Vec<u8>| -> Vec<u8> {
            edit_ctr_ciphertext(ciphertext_arg, &key, offset, newtext).unwrap_or(Vec::<u8>::new())
        };

    // Recover original plaintext
    let recovered_keystream =
        editing_api_call(&ciphertext, 0, &b"\x00".repeat(ciphertext.len()).to_vec());
...
fn challenge_26() {
    ...
    let target_keystream: Vec<u8> = zip(
        plaintext.iter(),
        ciphertext.chunks(16).nth(2).unwrap().iter(),
    )
    .map(|(a, b)| a ^ b)
    .collect();
    ...
    println!(
        "Does string contain an admin profile? : {}",
        find_admin_info(&decrypted_str)
    );
}
...
fn challenge_27() {
    ...
    let recovered_key: Vec<u8> = xor_bytearrays(
        &decrypted.chunks(16).nth(0).unwrap().to_vec(),
        &decrypted.chunks(16).nth(2).unwrap().to_vec(),
    );
...
fn challenge_29() {
    let my_message = BitVec::new_from_bytearray(
    ...
    let new_mac =
        sha1_length_extension_attack(&my_message, &initial_mac, &my_custom_message, key_len);
...
fn challenge_30() {
    ...
    let initial_mac = generate_mac(&my_message.to_bytearray());
    let new_mac =
        md4_length_extension_attack(&my_message, &initial_mac, &my_custom_message, key_len);
    println!("Initial MAC: {}\nNew MAC: {}", &initial_mac, &new_mac);
...
async fn challenge_31() {
    async fn validate_signature(Query(fileinfo): Query<FileInfo>) -> impl IntoResponse {
        ...
        if insecure_compare(
            &hextobytearray(&fileinfo.signature),
            &hextobytearray(&actual),
        ) {
            Response::builder()
