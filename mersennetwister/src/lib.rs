...
impl MT19937Gen {
    ...
    fn initialize(&mut self) {
        ...
        for ctr in 1..self.n {
            let res: u64 = (f as u64
                * (self.mt[ctr - 1] ^ (self.mt[ctr - 1] >> (self.w - 2))) as u64)
                + ctr as u64;
    ...
    fn twist(&mut self) {
        for ctr in 0..self.n {
            let x: u32 = (((self.mt[ctr] & self.upper) as u64
                + (self.mt[(ctr + 1) % self.n] & self.lower) as u64)
                & 0xFFFFFFFF)
                .try_into()
                .unwrap();
            let mut x_a: u32 = x >> 1;
...
mod tests {
    ...
    fn test_mt19937_rng_uniformity() {
        ...
        let lower_count: f32 = rand_floats
            .iter()
            .map(|&x| if x < 0.5 { 1.0f32 } else { 0.0f32 })
            .sum();
        let upper_count: f32 = rand_floats
            .iter()
            .map(|&x| if x >= 0.5 { 1.0f32 } else { 0.0f32 })
            .sum();
        assert_eq!(
            (lower_count / (num_floats / 100) as f32).round(),
            (upper_count / (num_floats / 100) as f32).round()
        );
    }
